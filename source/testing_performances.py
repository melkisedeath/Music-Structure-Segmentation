...
for file in os.listdir(MATCHSCORE_DIR):
	if file.endswith("match"):
		...
		X = structure_analysis(note_array, duration, step)
		...
		B = PCA(n_components=3, svd_solver='auto').fit_transform(Y)

		im = Image.fromarray(low_rank_SSM)
